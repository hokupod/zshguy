...
test_backend_defaults_to_lms() {
  local backend

  unset ZSHGUY_BACKEND

  if ! assert_helper_available _zshguy_backend "backend"; then
    return 1
  fi

  if ! backend="$(_zshguy_backend)"; then
    print -ru2 -- "FAIL: backend helper invocation failed"
    return 1
  fi

  assert_eq "lms" "$backend" "backend defaults to lms" || return 1
}

test_build_ollama_args_preserves_special_characters() {
  local ollama_args
  local ollama_prompt
  local -a expected_ollama_argv
  local expected_ollama_args
  local system_prompt=$'sys with spaces "quotes" * glob\nline'
  local user_prompt=$'user with spaces \'single quotes\' * glob\nline'
  local ZSHGUY_MODEL="test-model"

  if ! assert_helper_available _zshguy_build_ollama_args "build ollama args"; then
    return 1
  fi

  if ! ollama_args="$(_zshguy_build_ollama_args "$system_prompt" "$user_prompt")"; then
    print -ru2 -- "FAIL: build ollama args helper invocation failed"
    return 1
  fi
...
User request:
${user_prompt}"
  expected_ollama_argv=(run test-model "$ollama_prompt" --hidethinking)
  expected_ollama_args="${(@q)expected_ollama_argv}"
  assert_eq "$expected_ollama_args" "$ollama_args" "build ollama args special characters" || return 1
}

test_run_ollama_with_model() {
  local ollama_output
  local ollama_capture_file
  local ollama_prompt
  local -a ollama_argv
  local -a expected_ollama_argv
  local ZSHGUY_MODEL="test-model"

  if ! assert_helper_available _zshguy_run_ollama "run ollama"; then
    return 1
  fi

  ollama_capture_file="$(mktemp "${TMPDIR:-/tmp}/zshguy-ollama.XXXXXX")" || return 1
  ollama() {
    printf '%s\0' "$@" >"$ollama_capture_file"
    print -r -- "generated text"
  }

  if ! ollama_output="$(_zshguy_run_ollama "sys" "user")"; then
    print -ru2 -- "FAIL: run ollama helper invocation failed"
    return 1
  fi

  assert_eq "generated text" "$ollama_output" "run ollama output" || return 1
  while IFS= read -r -d '' ollama_arg; do
    ollama_argv+=("$ollama_arg")
  done <"$ollama_capture_file"
  rm -f "$ollama_capture_file"
  ollama_prompt=$'sys\n\nUser request:\nuser'
  expected_ollama_argv=(run test-model "$ollama_prompt" --hidethinking)
  assert_array_eq "run ollama argv with model" expected_ollama_argv ollama_argv || return 1
}

test_run_ollama_requires_model() {
  local ollama_output
  ...
  if ollama_output="$(_zshguy_run_ollama "sys" "user")"; then
    print -ru2 -- "FAIL: run ollama should reject an unset model"
    return 1
  fi

  assert_eq "ZSHGUY_MODEL is required when ZSHGUY_BACKEND=ollama" "$ollama_output" "run ollama requires model" || return 1
}

test_run_model_selects_ollama() {
  local model_output
  local ZSHGUY_BACKEND="ollama"
  local ZSHGUY_MODEL="test-model"

  if ! assert_helper_available _zshguy_run_model "run model"; then
    return 1
  fi

  ollama() {
    print -r -- "generated by ollama"
  }

  if ! model_output="$(_zshguy_run_model "sys" "user")"; then
    print -ru2 -- "FAIL: run model ollama selection failed"
    return 1
  fi

  assert_eq "generated by ollama" "$model_output" "run model selects ollama" || return 1
}

test_run_model_rejects_unknown_backend() {
  local model_output
  local ZSHGUY_BACKEND="unknown"
  ...
  if model_output="$(_zshguy_run_model "sys" "user")"; then
    print -ru2 -- "FAIL: run model should reject an unknown backend"
    return 1
  fi
...
test_run_ollama_failure_returns_stderr() {
  ...
  ollama() {
    print -ru2 -- "ollama boom"
    print -ru2 -- "additional detail"
    return 1
  }

  if ollama_output="$(_zshguy_run_ollama "sys" "user")"; then
    print -ru2 -- "FAIL: run ollama should return failure"
    return 1
  fi

  assert_eq "ollama boom" "$ollama_output" "run ollama failure returns first stderr line" || return 1
}
...
test_generation_error_message_uses_selected_backend() {
  local message
  local ZSHGUY_BACKEND="ollama"
  ...
  if ! message="$(_zshguy_generation_error_message "boom")"; then
    print -ru2 -- "FAIL: generation error message helper invocation failed for ollama"
    return 1
  fi

  assert_eq "[zshguy] ollama failed: boom. Continue typing to dismiss." "$message" "generation error message uses selected backend" || return 1
}
...
main() {
  ...
  run_test test_backend_defaults_to_lms
  run_test test_build_ollama_args_preserves_special_characters
  run_test test_run_ollama_with_model
  run_test test_run_ollama_requires_model
  run_test test_run_model_selects_ollama
  run_test test_run_model_rejects_unknown_backend
  run_test test_run_ollama_failure_returns_stderr
  ...
  run_test test_generation_error_message_uses_selected_backend
